...
Resources: https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status
## HTTP Overview
...
# TCP and TLS Handshake Process

## Step 1: TCP Handshake

**1. SYN (Synchronize)**  
- **Client → Server:** “Hi, I want to open a connection. Here’s my initial sequence number (ISN).”

**2. SYN-ACK (Synchronize-Acknowledge)**  
- **Server → Client:** “Got it! Here’s my ISN, and I acknowledge your sequence number.”

**3. ACK (Acknowledge)**  
- **Client → Server:** “Acknowledged your sequence number. Connection is established!”

**Result:**  
- TCP connection is now ready, and data can start flowing reliably.

---

## Step 2: TLS Handshake Over TCP

Once TCP is established, the **TLS handshake** begins over this connection:

**1. ClientHello**  
- Client proposes TLS version, cipher suites, and sends a **client random** number.

**2. ServerHello + Certificate**  
- Server picks the cipher suite, sends a **server random** number, and its digital certificate.

**3. Key Exchange**  
- Both client and server generate the **symmetric session key** using:
  - Random numbers (client + server)
  - Pre-master secret or Diffie-Hellman exchange

**4. Finished**  
- Both sides confirm they have the same session key.
...
# HTTP Response Codes (Common & Important Ones)

| Status Code | Category | Meaning / Explanation |
|-------------|---------|----------------------|
| **100** | Informational | Continue: client should continue with request. |
| **101** | Informational | Switching Protocols: server switches protocols as requested. |
| **200** | Success | OK: request succeeded; response contains requested data. |
| **201** | Success | Created: resource created successfully. |
| **202** | Success | Accepted: request accepted but not yet processed. |
| **204** | Success | No Content: request succeeded but no body returned. |
| **301** | Redirection | Moved Permanently: resource has new permanent URL. |
| **302** | Redirection | Found / Temporary Redirect. |
| **303** | Redirection | See Other: redirect to a GET request on another URL. |
| **304** | Redirection | Not Modified: cached version can be used; no body returned. |
| **307** | Redirection | Temporary Redirect: repeat request at another URI using same method. |
| **308** | Redirection | Permanent Redirect: like 301, but method/body not changed. |
| **400** | Client Error | Bad Request: malformed syntax or invalid request. |
| **401** | Client Error | Unauthorized: authentication required or failed. |
| **403** | Client Error | Forbidden: server refuses action regardless of authentication. |
| **404** | Client Error | Not Found: resource does not exist. |
| **405** | Client Error | Method Not Allowed: method not supported for the resource. |
| **408** | Client Error | Request Timeout: client did not send request in time. |
| **429** | Client Error | Too Many Requests: client sent too many requests. |
| **500** | Server Error | Internal Server Error: generic server-side error. |
| **501** | Server Error | Not Implemented: server does not support the functionality. |
